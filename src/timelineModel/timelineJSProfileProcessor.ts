...
export default class TimelineJSProfileProcessor {
  ...
  public static generateTracingEventsFromCpuProfile(jsProfileModel: CPUProfileDataModel, thread: Thread) {
    ...
    for (let i = 0; i < samples.length; ++i) {
      ...
      const jsSampleEvent = new Event(
          DevToolsTimelineEventCategory, RecordType.JSSample,
          Phase.Instant, timestamps[i], thread);
   ...
   * @return {!Array<!SDK.TracingModel.Event>}
   */
  public static generateJSFrameEvents(events: Array<Event>): Array<Event> {
    /**
    ...
    const jsFrameEvents: Array<any> = [];
    const jsFramesStack: Array<any> = [];
    const lockedJsStackDepth: Array<any> = [];
    let ordinal = 0;
     ...
     */
    function filterStackFrames(stack: Array<any>): void {
      if (showAllEvents)
        return;
   ...
   */
  public static nativeGroup(nativeName: string): NativeGroups {
