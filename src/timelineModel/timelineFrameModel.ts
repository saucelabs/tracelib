...
export default class TimelineFrameModel {
  ...
  public frames(startTime?: number, endTime?: number): TimelineFrame[] {
      if (!startTime && !endTime)
          return this._frames
      const firstFrame = lowerBound(this._frames, startTime || 0, (time, frame): number => time - frame.endTime)
      const lastFrame = lowerBound(this._frames, endTime || Infinity, (time, frame): number => time - frame.startTime)
