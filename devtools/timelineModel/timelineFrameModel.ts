...
export default class TimelineFrameModel {
    ...
    public frames(startTime?: number, endTime?: number): TimelineFrame[] {
        ...
        const firstFrame = lowerBound(
            this._frames,
            startTime || 0,
            (time, frame): number => time - frame.endTime
        )
        const lastFrame = lowerBound(
            this._frames,
            endTime || Infinity,
            (time, frame): number => time - frame.startTime
        )
     ...
     * @param {number} startTime
     */
    public handleDrawFrame(startTime: number): void {
        if (!this._lastFrame) {
            this._startFrame(startTime)
            return
        }
    ...
    private _processCompositorEvents(event: Event): void {
        ...
        if (event.name === eventNames.BeginFrame) {
            ...
            this.handleNeedFrameChanged(
                timestamp,
                event.args['data'] && event.args['data']['needsBeginFrame']
            )
        }
    }
    ...
    private _addMainThreadTraceEvent(event: Event): void {
        ...
        if (!this._framePendingCommit && this._mainFrameMarkers.indexOf(event.name) >= 0) {
            this._framePendingCommit = new PendingFrame(
                this._lastTaskBeginTime || event.startTime,
                this._currentTaskTimeByCategory
            )
        }
        ...
        if (
            event.name === eventNames.BeginMainThreadFrame &&
            event.args['data'] &&
        ...
        if (
            event.name === eventNames.Paint &&
            event.args['data']['layerId'] &&
            TimelineData.forEvent(event).picture &&
            this._target
        ) {
        ...
        if (
            event.name === eventNames.CompositeLayers &&
